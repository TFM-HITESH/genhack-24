...
const StayTunedComponent = () => {
  ...
  const targetTime = new Date("2024-09-27T08:00:00");
  
  // Update the difference every second
  useEffect(() => {
    const calculateDifference = () => {
      const now = new Date();
      const difference = Math.floor((targetTime.getTime() - now.getTime()) / 1000);
  ...
  const days = Math.floor(differenceInSeconds / 86400).toString().padStart(2, "0");
  const hours = Math.floor((differenceInSeconds % 86400) / 3600).toString().padStart(2, "0");
